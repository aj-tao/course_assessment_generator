#!/usr/bin/env python
# -*- coding: utf-8 -*-

import os, sys, csv
from pdfrw import PdfReader, PdfWriter
from latexcompiler import LC

# This function replaces [data, ...] from old_csv to [data, new_value] in new_csv.
# If [data, ...] isn't in old_csv anyways, we still make a new entry in new_csv with that pair.
def new_csv(data, new_value, old_csv, new_csv):
    new_row = dict()

    with open(old_csv, mode='r+', newline='') as oldcsvfile:
        reader = csv.reader(oldcsvfile, delimiter=',', quotechar=',')

        for row in reader:
            if row[0] == data:
                new_row[row[0]] = new_value
            else: 
                new_row[row[0]] = row[1]
        if data not in new_row.items(): # If it doesn't exist already...
            new_row[data] = new_value
        new_row = [[k,v] for k,v in new_row.items()]

    with open(new_csv, 'w+') as newcsvfile:
        writer = csv.writer(newcsvfile, delimiter=',', quotechar=',', quoting=csv.QUOTE_MINIMAL)

        for row in new_row:
            writer.writerow(row) 

# This function returns a string, which is the respective field associated
# to the data in exam_attributes.csv
def csv_access_field(data):
    with open('quiz/quiz_attributes.csv', mode='r+', newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',', quotechar=',')

        for row in reader:
            if row[0] == data:
                return row[1]
            
def exit_msg():
    print("You can only input the following flags: \n")
    print('\033[1m' +"-g:"+'\033[0m'+
          " generates quiz.tex and template problem_i.tex files in the directory quiz/ only from your quiz_attribute.csv file. \n")
    print('\033[1m' +"-s:"+'\033[0m'+
          " creates quiz.pdf from the files in the directory quiz/. \n")
    print('\033[1m' +"-m:"+'\033[0m'+
          " creates quiz_content_print_x_copies.pdf and quiz_first_two_pages.pdf from the files in the directory quiz/. \n")
    exit()
            
# This function creates a template "quiz.tex" just from the quiz_attributes.csv data,
# as well as template "quiz/problem_i.tex" data (if it hasn't been created yet).
def generate():
    quiz_problem_count = int(csv_access_field('quizproblemcount'))
    quiz_problem_weight = int(csv_access_field('quizproblemweight'))
    problem_dir = 'quiz/'

    # Modify grading.tex through ea.csv
    new_csv('documentcopy','~','quiz/quiz_attributes.csv','qa.csv')
    new_csv('quiztotal',quiz_problem_count*quiz_problem_weight,'qa.csv','qa.csv')

    # If it's just a single problem...
    instructions = '{instructions.tex}'
    if quiz_problem_count == 1:
        instructions = '{instructions_single.tex}'

    # Create the quiz.tex file
    with open("quiz.tex",'w+') as file:
        lines = ['\\input{quiz_preamble.tex}\n', 
                 '\\begin{document}\n', '\n', 
                 '%Title\n', 
                 '\\maketitle\n', '\n', 
                 '% Page instructions\n', 
                 f"\\input{instructions}\n", 
                 '\n']
        for line in lines:
            file.write(line)
        for i in range(quiz_problem_count):
            # We also create each of the problem files
            # as we reference them in the quiz.tex file
            problem = 'problem_'+str(i+1)+'.tex'
            lines = ['\\begin{tcolorbox}[colback=white,colframe=black,opacityframe=0,colbacktitle=black,title={\\textbf{Problem '+
                     str(i+1)+
                     ' [\\quizproblemweight\\ points]}}]\n', 
                     'Insert problem statement here!\n', 
                     '\\end{tcolorbox}\n']
            
            if os.path.isfile(problem_dir+problem): 
                # The problem_i.tex file is already in the directory 'quiz/'
                sys.stdout.write('Do you want to reset '+problem_dir+problem+'? (y/n) \n')
                join = sys.stdin.readline()
                if join[0] in ['y','Y']:
                    with open(problem_dir+problem, 'w+') as problem_file:
                        for line in lines:
                            problem_file.write(line)
            else: 
                # The problem_i.tex file isn't in the directory 'quiz/'
                # so we create a new one
                with open(problem_dir+problem, 'w+') as problem_file:
                    for line in lines:
                            problem_file.write(line)
                    
            file.write('\\input{'+problem_dir+problem+'} \n')
        lines = ['% Page grading','\n',
                 '\\input{grading.tex}','\n',
                 '\\newpage','\n',
                 '\\begin{center}\n',
                 'Blank page for additional work',
                 '\\end{center}\n','\n',
                 '\\end{document}\n']
        for line in lines:
            file.write(line)
    exit()


def single():
    quiz_problem_count = int(csv_access_field('quizproblemcount'))
    quiz_problem_weight = int(csv_access_field('quizproblemweight'))

    # Replacing the directory for printing
    os.system('rm -rf to_be_printed')
    os.mkdir('to_be_printed')

    # Modify grading.tex through ea.csv
    new_csv('documentcopy','~','quiz/quiz_attributes.csv','qa.csv')
    new_csv('quiztotal',quiz_problem_count*quiz_problem_weight,'qa.csv','qa.csv')

    # Just compile quiz.tex as is
    LC.compile_document(tex_engine = 'pdflatex', 
                        bib_engine = 'biber', 
                        no_bib = True, 
                        path = 'quiz.tex', 
                        folder_name = '.aux_files')
    os.system('mv quiz.pdf to_be_printed')

    # Remove the random files
    os.system('rm -rf .aux_files')

def multiple():
    document_total = int(csv_access_field("documenttotal"))
    quiz_content_pdf = 'quiz_multiple_copies.pdf'

    # Replacing the directory for printing
    os.system('rm -rf to_be_printed')
    os.mkdir('to_be_printed')
    
    # Want to iterate over document_total times. 
    # For each iteration, 
    # 1) write the document field at the end of the new csv file
    # 2) with this new csv file, generate a new exam tex file
    # 3) compile the respective pdf for this exam tex file
    # then combine all of these pdfs into one pdf exam_first_two_pages.pdf

    merger = PdfWriter()
    
    for i in range(document_total):
        new_path_tex = 'quiz'+str(i+1)+'.tex'
        new_path_pdf = 'quiz'+str(i+1)+'.pdf'

        new_csv('documentcopy',i+1,'qa.csv','qa.csv')
        os.system('cp quiz.tex '+new_path_tex)
        LC.compile_document(tex_engine = 'pdflatex', 
                            bib_engine = 'biber', 
                            no_bib = True, 
                            path = new_path_tex, 
                            folder_name = '.aux_files')
        merger.addpages(PdfReader(new_path_pdf).pages)
        os.remove(new_path_tex)
        os.remove(new_path_pdf)
        
    merger.write("to_be_printed/"+quiz_content_pdf)

    # Remove the random files
    os.system('rm -rf .aux_files')
    

def main():
    if input == '-g':
        generate()
    if input == '-s':
        single()
    elif input == '-m':
        multiple()
    else:
        exit_msg()

if __name__ == "__main__":
    if len(sys.argv) != 2:
        exit_msg()
    input = sys.argv[1]
    main()
